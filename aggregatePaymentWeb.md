...
|risk_item|Required|String| This parameter is used for payment risk control, all required parameters should be included in the value of ```risk_item``` in json format. e.g. {"delivery_addr_city":"xxxxxx","delivery_addr_province":"xxxxxx","delivery_cycle":"12小时内","delivery_phone":"xxxxxxxxxxx","frms_ware_category":"4016","goods_count":"5","logistics_mode":"2","user_info_bind_phone":"xxxxxxxxxxx","user_info_dt_register":"20171115122536","user_info_mercht_userno":"17432"}| 
...
You are not required to send response for synchronous notification.

## Asynchronous notification

Once the merchant payment request is processed, LianLian will send the results to the notification URL ```notify_url``` via server-side HTTP requests, which is wrote into *HttpInputStream*. You can choose one way to obtain parameters depedning on your programming language:
